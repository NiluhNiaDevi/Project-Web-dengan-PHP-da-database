...
// Proses pengiriman formulir register
if (isset($_POST['send'])) {
    $name = $_POST['name'];
    $phone = $_POST['phone'];
    $email = $_POST['email'];
    ...
    if (mysqli_num_rows($checkEmailResult) > 0) {
        ...
        $insertQuery = "INSERT INTO register (name, phone, email, password) VALUES ('$name', '$phone', '$email', '$password')";
...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($loginResult) > 0) {
        $_SESSION['email'] = $email; // Simpan email pengguna dalam sesi
        header("Location: home.php"); // Mengarahkan pengguna ke halaman dashboard setelah login berhasil
        exit();
    } else {
